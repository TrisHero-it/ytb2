...
<body>
    <div class="container">
        <div class="card">
            ...
            @error('username')
                <p style="color: #ff5050; text-align: center; margin-bottom: 16px; font-size: 14px;">{{ $message }}</p>
            @enderror
            <form class="card_form" method="POST" action="{{ route('login') }}">
                @csrf
                <div class="input">
                    <input type="text" name="username" class="input_field" value="{{ old('username') }}" required />
